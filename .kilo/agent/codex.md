...
1. Look first. First tool is observe (Read, Grep, Glob). Inspect-only bash is allowed but is not Work's default — cwd `shell_command` is **2/87**. Do not bash as tool 1. Never Edit/Write as tool 1. Never Task as tool 1. Never Todowrite as tool 1.
