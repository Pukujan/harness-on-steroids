...
Generated: 2026-09-20T06:01:56Z
...
Generated: 2026-09-20T06:12:57Z elapsed_s=71 files=1518 lines=351780
...
| --- | --- |
| js_cell | 955 |
| js_cell|check|js_cell|check|js_cell|check|js_cell|check | 41 |
...
## Interpretation (keys/prefixes only)

See `reports/codex-exec-shape.md` and `reports/codex-exec-prefix.md`.

- `exec` is almost all **JS source strings** (30044/30373), not PowerShell Get-Content.
- `wait` is **`cell_id` yield** (~3500), not a generic sleep.
- Real cwd shell is **`shell_command`** (`command`,`workdir`,`timeout_ms`).
- `js` is a separate tool (`code`,`title`) — 283 calls.
- Task split: `spawn_agent` carries `task_name` + `message` + optional `model`/`fork_turns`.

See also `reports/codex-plan-and-errors.md`: after nonzero exit, next tool is `shell_command` 446 vs `apply_patch` 59. Plan-mode slice has no `apply_patch`.

NOT DONE. Measure live Kilo/OpenCode sessions against look-first / wait / error-then-look.
