...
Source: `reports/codex-gold-behavior.md`, `reports/codex-exec-shape.md`, `reports/codex-exec-prefix.md`, `reports/codex-itemtype-shapes.md`. Hashed corpus **1521** files (was 1518). Not SWE-bench papers.
...
**Primary gold slice is `codex_work_desktop` (ChatGPT Work),** 87 sessions (hashed corpus 1521). `apply_patch` in **1** session. Multi-agent send/spawn **26**. VS Code Codex (n=16) is the shell↔patch sandwich; do not let that minority overwrite Work.
