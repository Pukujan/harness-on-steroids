...
5. **Split only if multi-piece.** After a look burst, Task / send to a **named target**. Do not Todowrite — Work `update_plan` is **0**. One slice does not spawn. Never Task as tool 1. Never Todowrite as tool 1. `spawn_agent` is rare (2/85).
